...
function SuccessStories() {
    ...
    useEffect(() => {
        const stories_data = async () => {
            try {
                const response = await axios.get("http://localhost:4848/stories");
    ...
    return (
        <div className='crowd_cat_main'>
            ...
            <div className='crowd_cat absolute w-full h-[90.4%]'>
                <h1 className='absolute left-[6%] text-blue-100 text-5xl font-semibold uppercase m-2 mb-5'>Stories</h1>
 ...
 */}
                        <div className='crowd_home_trending_fundraiser_items flex text-white flex-wrap cursor-pointer h-[50%]'>
                            {storiesData.map(details => (
                                <Link to={`/StoriesDetails/${details.storiesName}`} className='m-5 bg-blue-500 p-3 rounded-lg w-80'>
                                    {/* <img src="/images/n4.jpg" alt="" className='h-52 w-full' /> */}
                                    <img src={details.storiesImage?.substring(14)} alt="" className='h-52 w-full' />
                                    <div className='p-3'>
                                        <h4 className='text-blue-100 text-xl uppercase text-center m-2 mb-5'>"{details.storiesName}"</h4>
                                        <div className='flex justify-center my-3 ml-3'>
                                            <Link to={"/Payment/" + details.storiesName} className='bg-blue-950 hover:bg-blue-200 text-white hover:text-blue-950 px-4 py-2 h-10 mt-1 rounded cursor-pointer mr-5 duration-500'>Donate</Link>
                                        </div>
                                    </div>
                                </Link>
                            ))}
